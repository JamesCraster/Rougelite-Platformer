...
void loadLevelFromLevelVector(std::vector<std::vector<int>> levelVector, sf::Vector2f tileSize, std::vector<Spike*> & spikeVector, std::vector<sf::RectangleShape*> & tileVector){
    for (int x = 0; x < levelVector.size(); x++) {
        for(int y = 0; y < levelVector[x].size(); y++){
            if(levelVector[x][y] == 1 &&((x-1 > 0 && levelVector[x-1][y] == 0)||(x+1 > levelVector.size() && levelVector[x+1][y] == 0)||(y-1 > 0 && levelVector[x][y-1] == 0)||(y+1 > levelVector[0].size() && levelVector[x][y+1] == 0))){
                if(y-1 > 0 && levelVector[x][y-1] != 1){
                    ...
                    if(tileVector.size() > 0){
                        ...
                        tileVector[tileVector.size()-1]->setOutlineColor(sf::Color(24,118,118));
                        tileVector[tileVector.size()-1]->setOutlineThickness(1);
                        tileVector[tileVector.size()-1]->setSize(tileVector[tileVector.size()-1]->getSize() + sf::Vector2f(0, tileSize.y-1));
...
std::vector<std::vector<int>> loadLevelFromChunkChain(ChunkChain chunkChain, sf::Vector2f tileSize, std::vector<Spike*> &spikeVector, std::vector<sf::RectangleShape*>& tileVector,std::vector<sf::RectangleShape*> & showTileVector){
    for(int i = 0; i < tileVector.size(); i++){
        delete tileVector[i];
    }
    spikeVector =  std::vector<Spike*>();
    tileVector = std::vector<sf::RectangleShape*>();
    showTileVector = std::vector<sf::RectangleShape*>();
    
    auto levelVector = chunkChain.levelVector;
    for (int x = 0; x < levelVector.size(); x++) {
        for(int y = 0; y < levelVector[x].size(); y++){
            
            if(levelVector[x][y] == 1 &&((x-1 > 0 && levelVector[x-1][y] != 1)||(x+1 < levelVector.size() && levelVector[x+1][y] != 1)||(y-1 > 0 && levelVector[x][y-1] != 1)||(y+1 < levelVector[0].size() && levelVector[x][y+1] != 1))){
                //if(y-1 > 0 && levelVector[x][y-1] != 1){
                    tileVector.push_back(new sf::RectangleShape);
                    tileVector[tileVector.size()-1]->setPosition(x *tileSize.x, y*tileSize.y);
                    tileVector[tileVector.size()-1]->setFillColor(sf::Color(0,0,0,0));
            ...
            if(levelVector[x][y] == 1 || levelVector[x][y] == 2){
                showTileVector.push_back(new sf::RectangleShape);
                showTileVector[showTileVector.size()-1]->setPosition(x *tileSize.x, y*tileSize.y);
                showTileVector[showTileVector.size()-1]->setFillColor(sf::Color(0,0,0,0));
                showTileVector[showTileVector.size()-1]->setOutlineColor(sf::Color(24,118,118));
                showTileVector[showTileVector.size()-1]->setOutlineThickness(1);
                showTileVector[showTileVector.size()-1]->setSize(sf::Vector2f(tileSize.x-1, tileSize.y-1));
            }
